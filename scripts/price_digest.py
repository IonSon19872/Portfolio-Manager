...
Finnhub calls per run: 5 calls × 20 holdings = 100 calls (~100 s at 1 s/call).
"""
...
def build_snapshot(cfg: dict) -> dict:
    ...
    for holding in cfg["portfolio"]["stocks"]:
        ...
        if "error" not in data:
            ...
            log.info(f"    ✓ €{data['price_eur']}  ({data['change_pct']:+.2f}%)  value €{data['value_eur']}")
        else:
            log.warning(f"    ✗ {data['error']}")
    ...
    for item in snapshot["stocks"] + snapshot["etfs"]:
        ...
        if prev and prev > 0:
            ...
            if abs(move) >= threshold:
                ...
                send_email(
                    f"🚨 {ticker} {direction} {abs(move):.1f}%",
...
With 1 s between each call, 20 stocks × 5 calls = ~100 s total runtime.
